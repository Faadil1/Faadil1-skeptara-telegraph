...
Write-Host "Skeptara T2 secure local launcher"
Write-Host "1. Keep this PowerShell window open."
Write-Host "2. Switch to the fresh bounded testnet burner and copy ONLY its exported EVM private key."
Write-Host "3. Do NOT type or paste the key into PowerShell."
Write-Host "4. This helper detects the clipboard key automatically and clears the clipboard before running T2."
Write-Host ""
...
while ((Get-Date) -lt $deadline) {
    try {
        ...
        if ($candidate -match '^[0-9a-fA-F]{64}$') { $candidate = "0x$candidate" }
...
if (-not $pk) {
    Write-Host "No valid EVM private key was detected within 90 seconds." -ForegroundColor Red
    exit 3
}

Write-Host "VALID EVM private-key format detected. Launching T2 without printing the key." -ForegroundColor Green
